...
Any changes that impact the user experience in any way must be accompanied
by changes to the documentation.

### Testing

Any changes (other than documentation) must be accompanied by tests to be
accepted. Any change to fix an error must be accompanied by a regression test
that ensures the error does not recur in the future.

To run the test suite, run `npm run test` in the root directory of this
repository.  Any new commands needed to run the test suite must be added
to `package.json`.
